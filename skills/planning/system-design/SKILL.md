...
| **統一語言表** | 同上文件，user stories 之前那張表 | §2 欄位命名沒有依據 → 同概念多名（`report_date` vs `date`），就是那條「踩過的坑」 |
...
> **這張表就是 DDD 的限界上下文（bounded context），只是用中文寫。**
> 刻意點名是因為：一個工程師照著填十次「不負責什麼」，如果沒人告訴他這叫什麼，
> 他下個專案不會自己想到要畫。概念要能遷移，得先有名字。
> 同理，§2 的「正規化放在服務邊界，不要藏在查詢深處」是**防腐層**（anti-corruption layer）；
> `requirement` Step 2 的統一語言表是**ubiquitous language**，也是這張表的上游輸入 ——
> 名詞在那裡釘死，邊界才切得穩。

#### §3.1 目錄結構（邊界的實體形式）

**光有表格不夠 —— 邊界最後要能在檔案系統上看見。** 表格上切得很漂亮、程式碼還是
`controllers/ models/ services/` 三層平鋪，等於沒切：任何人都能從任何地方 import 任何東西，
「不負責什麼」沒有任何物理力量去執行它。
...
現況                          目標
src/                          src/
├── controllers/              ├── ingest/          ← 一個模組一個資料夾
│   ├── ingest.py             │   ├── service.py
│   └── report.py             │   └── models.py
├── models/                   ├── report/
│   ├── upload.py             │   ├── service.py
│   └── report.py             │   └── models.py
└── services/                 └── shared/          ← 跨模組共用才放這
    ├── ingest.py                 └── db.py
    └── report.py
```
...
- **按領域切，不要按技術分層。** 資料夾名要對得上 §3 表格的「模組」欄，一格一個資料夾。
  技術分層（controller/model/service）是每個模組**內部**的事，不是頂層。
- **`shared/` 要有門檻**：只有兩個以上模組都真的在用才准放。一個模組專用的東西放進 `shared/`，
  就是把邊界偷偷拆掉。
- **判準：新人拿到需求「工單要能退回」，能不能只看資料夾名就知道要開哪個檔？**
  答不出來就是切錯了，回 §3 表格重切。
- **現況那棵樹要實查**（`tree -L 2` 或 `ls -R`），不要憑印象畫。兩棵樹的差就是重構範圍，
  由 `planning-with-files` 拆成任務。
