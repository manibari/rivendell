...
description: >
  ...
  state transitions, integrating sales-customer-intel reports.
  TRIGGER: 「新 client」「presales」「這個可以 archive / lost 了」, company-level
  prospect naming, or cd-ing into an `01_presales` directory.
  SKIP: deep one-off company research (sales-customer-intel); CRM record edits
  (sales-crm-projection); a sales deck (sales-material / pitch-deck).
...
2. **Enrich** — pull a sales-customer-intel report into the folder so the prospect file
...
- **One folder per prospect, slug is the key**: pick a stable `<client-slug>` up
  front; renaming later orphans the sales-customer-intel cross-references.
